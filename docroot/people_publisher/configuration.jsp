...
<aui:form action="<%= configurationActionURL %>" method="post" name="fm" onSubmit="event.preventDefault();">
	...
	<aui:input name="userOrder" type="hidden" value="-1" />
	...
	<liferay-util:buffer var="selectStyle">
		<aui:fieldset label="user-selection">
				...
				>
					...
					>
						<liferay-portlet:actionURL portletConfiguration="true" var="deleteURL">
							...
							<portlet:param name="scopeId" value="<%= peoplePublisher.getScopeId(group, scopeGroupId) %>" />
			...
			<div class="select-user-selector">
				<liferay-ui:icon-menu cssClass="select-existing-selector" direction="right" icon='<%= themeDisplay.getPathThemeImages() + "/common/add.png" %>' message="select" showWhenSingleIcon="<%= true %>">
					...
					<c:if test="<%= GroupLocalServiceUtil.getGroupsCount(company.getCompanyId(), Layout.class.getName(), layout.getGroupId()) > 0 %>">
						...
						<%
						PortletURL layoutSiteBrowserURL = PortletURLFactoryUtil.create(request, com.liferay.portal.util.PortletKeys.SITE_BROWSER, PortalUtil.getControlPanelPlid(company.getCompanyId()), PortletRequest.RENDER_PHASE);
					...
					<c:if test="<%= !types.isEmpty() %>">
						...
						<%
						PortletURL siteBrowserURL = PortletURLFactoryUtil.create(request, com.liferay.portal.util.PortletKeys.SITE_BROWSER, PortalUtil.getControlPanelPlid(company.getCompanyId()), PortletRequest.RENDER_PHASE);
	...
	<c:choose>
		<c:when test='<%= selectionStyle.equals("manual") %>'>
			<liferay-util:include page="/people_publisher/configuration_manual.jsp" servletContext="<%= application %>" />
		</c:when>
		<c:when test='<%= selectionStyle.equals("dynamic") %>'>
			<liferay-util:include page="/people_publisher/configuration_dynamic.jsp" servletContext="<%= application %>" />
		</c:when>
	</c:choose>
</aui:form>
...
<aui:script>
	...
	function <portlet:namespace />moveSelectionDown(userOrder) {
		document.<portlet:namespace />fm.<portlet:namespace /><%= Constants.CMD %>.value = 'move-selection-down';
		document.<portlet:namespace />fm.<portlet:namespace />userOrder.value = userOrder;

		submitForm(document.<portlet:namespace />fm);
	}

	function <portlet:namespace />moveSelectionUp(userOrder) {
